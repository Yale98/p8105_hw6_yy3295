...
Show a plot of model residuals against fitted values – use add_predictions and add_residuals.
...
Compare the hypo model to the model using length at birth and gestational age as predictors (main effects only), and the model using head circumference, length, sex, and all interactions (including the three-way interaction) between these.
...
From the plot we can tell that the hypothesized model is the most appropriate model among all the three models as it has the lowest RMSE. Though, the three way interaction model also has low RMSE,  the hypothesized model is still the most appropriate one.
